...
export class UserFavoritePlaygroundController {
  constructor(private readonly userFavoritePlaygroundService: UserFavoritePlaygroundService) {}
  ...
  @Post()
  async addFavorite(@Body() createUserFavoritePlaygroundDto: CreateUserFavoritePlaygroundDto) : Promise<UserFavoritePlayground>{
    return await this.userFavoritePlaygroundService.create(createUserFavoritePlaygroundDto);
  }
  ...
  @ApiQuery({
    example : "user-favorite-playground?userId=:userId&playgroundId=:playgroundId"
  })
  @Delete()
  removeFavorite(@Query('userId') userId : string, @Query('playgroundId') playgroundId : string) {
    return this.userFavoritePlaygroundService.removeFavorite(userId, playgroundId);
  }
}
